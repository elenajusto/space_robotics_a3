...
# Configure
local_image_directory = r"advanced_1_raw_and_effects"  # Directory to save images locally
    ...
    def advanced_1_image_processing(self, image):
        ...
        match effect:
            ...
            case "blur": # median blur filter
                distorted_image = cv2.medianBlur(image,5) #applying median blur to reduce noise
            ...
            case _:  #default case if no effect matches
                self.get_logger().warn(f"Effect '{effect}' not recognised, no processing will be done.")
                return image  #no processing done
